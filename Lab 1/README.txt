...
retrieved the return value of the child if it had one by retrieving the child's exit status after the wait with WIFEXITED(). An if statement
is used to check if the child had a valid return value and outputs the CPID and retval. The code's output looks acceptable based on your 
example on Slack. This lab assignment was easier to do than lab0, unless I am doing something wrong that doesn't follow the way the fork
should be handled/presented.
